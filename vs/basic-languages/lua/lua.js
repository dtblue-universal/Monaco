...
define(["require", "exports"], function (require, exports) {
    ...
    exports.language = {
        defaultToken: '',
        tokenPostfix: '.lua',
        keywords: 'and break continue do else elseif end for function if in local not or repeat return then until while next type'.split(' '),
    ...
    function AddAutocompletes(autocompleteContainer, scope, delimiter) {
        for (const kind in autocompleteContainer) {
            ...
            if (typeof autocompletes === 'undefined' || (delimiter && kindToDelimiter[kind] && !kindToDelimiter[kind].includes(delimiter)))
                continue;

            if (autocompletes[0])
            {
                for (const name of Object.values(autocompletes)) AddSnippet(kind, name, { __scope__: scope });
